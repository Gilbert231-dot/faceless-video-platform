...
# on screen while it's being said: it fades + slides UP into the center, holds
# while the title is narrated, then fades + slides UP and away. Pure overlay
# inside segment 0's existing encode - no extra pass, negligible CPU on the
# Actions runner.
...
def compile_video(video_paths, audio_path, script, subtitle_path=None,
    ...
    if intro_frame and os.path.exists(intro_frame) and TITLE_INTRO:
        ...
        if total_words and title_words:
            ...
            t4 = title_secs
            t3 = max(t4 - TITLE_FADE_SEC, t2 + 0.2)  # fade-out never before fade-in ends
            ...
            # and captions below it. y slides the card in/out on entry/exit.
            frame_top = round(0.14 * OUTPUT_H)
            y_expr = (f"{frame_top}+{slide}*(1-clip((t-{t1:.3f})/{t2 - t1:.3f},0,1))"
                      f"-{slide}*clip((t-{t3:.3f})/{t4 - t3:.3f},0,1)")
            overlay_filter = (
                f"[1:v]scale={OUTPUT_W}:-1:flags=lanczos,"
                f"fade=t=in:st={t1:.3f}:d={TITLE_FADE_SEC:.3f}:alpha=1,"
